...
The data is cleaned up by selecting relevant columns and un-nesting, widening and hoisting the list-columns.
...
At this point, the data can be saved to disk as a CSV as a backup (since the API calls took several minutes to complete and this can avoid having to repeat these costly steps).
...
Now, the subset for only the bestseller information can be joined to the `gr` table.
